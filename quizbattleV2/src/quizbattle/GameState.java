...
/**
 * GameState — pure data + game-logic class.
 * No Swing imports. BattlePanel reads state fields and calls mutator methods.
 */
public class GameState {

    // ── Game Mode ─────────────────────────────────────────────────────────────
    public enum Mode { CLASSIC, BLITZ, COMPUTER }
    ...
    public int turnSeconds() {
        return switch (gameMode) {
            case BLITZ    -> 3;
            default       -> 10;   // CLASSIC and COMPUTER both use 10s
        };
    }

    public boolean isComputerMode() { return gameMode == Mode.COMPUTER; }
    public boolean isBotTurn()      { return isComputerMode() && currentTurn == 1; }
     ...
     * Returns true if the timer just hit zero (caller should handle timeout).
     */
    public boolean tickTurnTimer() {
        if (answered || gameOver) return false;
        ...
        if (timerTickAcc >= TICKS_PER_SECOND) {
            timerTickAcc = 0;
            turnSecondsLeft--;
            if (turnSecondsLeft <= 0) return true;
        }
        return false;
    }
    ...
    public boolean handleInput(int choice, boolean isP1, LogCallback log) {
        ...
        if (correct) {
            int pts = 1;
            if (isP1 && p1Double) {
                pts = 2; p1Double = false;
                log.log("✨ P1 Double Points activated!");
            } else if (!isP1 && p2Double) {
                pts = 2; p2Double = false;
                log.log("✨ " + (isComputerMode() ? "CPU" : "P2") + " Double Points activated!");
            }
            if (isP1) p1Pts += pts; else p2Pts += pts;

            boolean blocked = false;
            if (isP1 && p2Shield)  { p2Shield = false; blocked = true; log.log("🛡 " + (isComputerMode() ? "CPU" : "P2") + "'s Shield blocked the attack!"); }
            if (!isP1 && p1Shield) { p1Shield = false; blocked = true; log.log("🛡 P1's Shield blocked the attack!"); }
            if (!blocked) {
                if (isP1) p2HP -= 20; else p1HP -= 20;
            }
    ...
    public boolean useSkill(String skill, LogCallback log) {
        ...
        switch (skill) {
            case "strike" -> {
                boolean blocked = false;
                if (isP1 && p2Shield)  { p2Shield = false; blocked = true; log.log("🛡 " + (isComputerMode() ? "CPU" : "P2") + "'s Shield blocked Strike!"); }
                if (!isP1 && p1Shield) { p1Shield = false; blocked = true; log.log("🛡 P1's Shield blocked Strike!"); }
                if (!blocked) {
                    if (isP1) p2HP -= 20; else p1HP -= 20;
            ...
            case "drain" -> {
                if (isP1) { p2HP -= 20; p1HP = Math.min(100, p1HP + 20); }
                else       { p1HP -= 20; p2HP = Math.min(100, p2HP + 20); }
    ...
    // CONVENIENCE
    // ═════════════════════════════════════════════════════════════════════════
    public boolean isP1Turn()  { return currentTurn == 0; }
    public int  activePts()    { return isP1Turn() ? p1Pts : p2Pts; }
    public void advanceTurn()  { currentTurn = 1 - currentTurn; }

    @FunctionalInterface
    public interface LogCallback { void log(String msg); }
}